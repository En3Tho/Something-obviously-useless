    ...
    public sealed class ArrayPoolBufferWriter : IBufferWriter<byte>, IDisposable
    {
        ...
        private void CheckAndResizeBuffer(int sizeHint)
        {
            ...
            if (sizeHint > FreeCapacity)
            {
                ...
                var _newBuffer = ArrayPool<byte>.Shared.Rent(newSize);
                _buffer.CopyTo(_newBuffer.AsSpan());
        ...
        private static void ThrowInvalidOperationException_AdvancedTooFar(int capacity) => throw new InvalidOperationException($"BufferWriterAdvancedTooFar : {capacity}");

        private static void ThrowOutOfMemoryException(uint capacity) => throw new OutOfMemoryException($"BufferMaximumSizeExceeded: {capacity}");
        public void Dispose()
        {
            ArrayPool<byte>.Shared.Return(_buffer);
        }
