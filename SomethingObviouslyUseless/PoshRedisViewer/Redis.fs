...
module RedisReader =
    ...
    let getAllKeys (multiplexer: IConnectionMultiplexer) database = task {
        try
            let server = multiplexer.GetServer(multiplexer.Configuration)
            let! keys = server.KeysAsync(database) |> AsyncEnumerable.toResizeArray
            return
                keys
                |> Seq.map toString
                |> Seq.toArray
